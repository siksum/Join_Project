...
contract AggregatedTokenSwap {
    ...
    constructor(address payable _owner, IGST2 _gasToken, uint _fee) public {
    ...
    function aggregate( IERC20 fromToken, IERC20 toToken, uint tokensAmount, address[] memory callAddresses, bytes memory callDataConcat, uint[] memory starts, uint[] memory values, uint mintGasPrice, uint minTokensAmount, address payable referrer) public payable returns (uint returnAmount){
    ...
    function infiniteApproveIfNeeded(IERC20 token, address to) external {
        if (address(token) != ETH_ADDRESS && token.allowance(address(this), to) == 0) {
    ...
    function audoRefundGas(uint startGas, uint mintGasPrice) private returns (uint freed) {
    ...
    function refundGas(uint tokensAmount) private returns (uint freed)
    {
        if (tokensAmount > 0) {
